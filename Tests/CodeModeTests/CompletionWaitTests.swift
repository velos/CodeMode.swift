...
/// Serialized: these assert on a dispatched block getting a thread, which is exactly
/// what the watchdog and event-loop suites make scarce when run alongside them.
@Suite(.serialized) struct CompletionWaitTests {
    ...
    @Test func completionWaitReturnsTheDeliveredValue() throws {
        // 60s, not 5: other suites block GCD workers for the length of a timer (the
        // synchronous bridge model holds a thread per running execution), so even an
        // immediate `global().async` can wait seconds for a thread on a loaded
        // runner. The assertion is that the delivered value comes back, not how
        // fast — the thread pressure itself is what the async ABI (#9) removes.
    ...
    @Test func completionWaitToleratesALateCallback() throws {
        ...
        // Generous on purpose: other suites block GCD workers for the length of a
        // timer, so a 0.2s dispatch can be starved for seconds on a loaded runner.
        // The assertion is that the late callback lands at all.
